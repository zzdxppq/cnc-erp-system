...
2. **Available Agents**: Call `orchestrix-list-agents` tool (from `orchestrix` MCP server) to verify agents are loaded
...
1. Check `opencode.jsonc` MCP server configuration
2. Verify the remote MCP server URL is correct
3. Verify the license key header is configured correctly
